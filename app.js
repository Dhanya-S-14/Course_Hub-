...
// YouTube Courses
let youtubeCoursesData = [];

async function fetchYouTubeCourses() {
...
function displayYoutubeCourses(courses) {
    const grid = document.getElementById('youtubeGrid');
    if (!grid) return;
    grid.innerHTML = '';
    
    if (!courses || courses.length === 0) {
        grid.innerHTML = '<p class="loading-courses">No courses found.</p>';
        return;
    }
    ...
    courses.forEach(course => {
        ...
        card.innerHTML = `
            <img src="${course.thumbnail || 'https://img.youtube.com/vi/dQw4w9WgXcQ/mqdefault.jpg'}" alt="${course.title}">
            <div class="youtube-card-content">
                <h3>${course.title}</h3>
                <p class="channel"><i class="fa-brands fa-youtube"></i> ${course.channelName || 'Unknown'}</p>
                <span class="category-tag">${course.category || 'General'}</span>
                <a href="${course.videoUrl}" target="_blank" class="watch-btn">
...
function filterYoutubeCourses(category) {
    const buttons = document.querySelectorAll('.yt-filter');
    ...
    if (category === 'all') {
        ...
        const filtered = youtubeCoursesData.filter(c => c.category === category);
        displayYoutubeCourses(filtered);
    }
}

function scrollToYouTubeCourses() {
    const section = document.getElementById('youtubeSection');
    const btnWrapper = document.querySelector('.youtube-btn-wrapper');
    if (section) {
